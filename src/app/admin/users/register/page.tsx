...
import { Gavel, Users, Building, Scale, Eye, EyeOff, AlertCircle, Loader2, CheckCircle, User } from 'lucide-react';
...
const RegistrationSystem = () => {
  ...
  const phoneRef = useRef<HTMLInputElement>(null); // Added for public users
  const courtTypes = [
...
const getFormData = (): RegistrationFormData => {
  return {
    ...
    courtType: (activeTab === 'judge' || activeTab === 'registrar' || activeTab === 'clerk') 
      ? (courtTypeRef.current?.value as CourtType) 
      : undefined,
    courtLocation: (activeTab === 'judge' || activeTab === 'registrar' || activeTab === 'clerk') 
      ? courtLocationRef.current?.value 
      : undefined,
    ...
    phone: activeTab === 'public' ? phoneRef.current?.value : undefined, // Added for public users
  ...
  const resetForm = () => {
    ...
    if (phoneRef.current) phoneRef.current.value = ''; // Added for public users
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    if (role === 'registrar') {
      ...
      role = 'admin';
      adminType = 'clerk';
    } else if (role === 'admin' && formData.adminType) {
    ...
    } else if (role === 'admin' && adminType) {
      profile = {
        adminType
      };
      
      // Add court information for registrar and clerk
      if (adminType === 'registrar' || adminType === 'clerk') {
        profile.courtType = formData.courtType;
        profile.courtLocation = formData.courtLocation;
      }
    } else if (role === 'public') {
      profile = {
    ...
    setTimeout(() => {
      const dashboardPath = getDashboardPath(role, adminType);
...
const getDashboardPath = (role: UserRole, adminType?: string): string => {
  if (role === 'admin' && adminType) {
    if (adminType === 'registrar') return '/registrars/dashboard';
    if (adminType === 'clerk') return '/clerk/dashbaord';
  }
  ...
  const PublicFields = () => (
    <div>
      ...
      <p className="text-sm text-gray-500 mt-2">
        Public users can search cases, file documents, and view published judgments.
      </p>
    </div>
  );
            ...
            <form onSubmit={handleSubmit}>
              <Tabs value={activeTab} onValueChange={handleTabChange}>
                ...
                <TabsList className="grid w-full grid-cols-6">
                  ...
                  <TabsTrigger value="registrar" className="flex items-center gap-2" disabled={isLoading}>
                    <Users className="h-4 w-4" />
                    Registrars
                  </TabsTrigger>
                  <TabsTrigger value="clerk" className="flex items-center gap-2" disabled={isLoading}>
                    <Users className="h-4 w-4" />
                    Clerks
                  </TabsTrigger>
                  ...
                  <TabsTrigger value="public" className="flex items-center gap-2" disabled={isLoading}>
                ...
                <div className="space-y-4 mt-6">
                  ...
                  {activeTab === 'lawyer' && (
                    ...
                    {activeTab === 'registrar' && (
                      <div className="border-t pt-4">
                        <h3 className="text-lg font-semibold mb-4 text-orange-600">Registrar Information</h3>
                        <div className="space-y-4">
                          <RegistrarFields />
                        </div>
                      </div>
                    ...
                    {activeTab === 'clerk' && (
                      <div className="border-t pt-4">
                        <h3 className="text-lg font-semibold mb-4 text-orange-600">Clerk Information</h3>
                        <div className="space-y-4">
                          <ClerkFields />
                        </div>
                      </div>
                    ...
                    {activeTab === 'public' && (
                      <div className="border-t pt-4">
                        <h3 className="text-lg font-semibold mb-4 text-orange-600">Public User Information</h3>
                        <div className="space-y-4">
                          <PublicFields />
                        </div>
                      </div>
                  ...
                  <div className="border-t pt-4 space-y-3">
                    ...
                    >
                      {isLoading ? (
                        ...
                        `Register ${activeTab === 'judge' ? 'Judge' : activeTab === 'lawyer' ? 'Lawyer' : activeTab === 'public' ? 'Public User' : 'Administrator'}`
                      )}
                    </Button>
          ...
          <CardContent>
            <div className="space-y-2 text-sm">
              ...
              <div className="flex items-start gap-2">
                <div className="w-2 h-2 bg-orange-600 rounded-full mt-2"></div>
                <div>
                  <strong>Public Users:</strong> Can search public cases, file documents, and view published judgments
                </div>
              </div>
